...
file_paths = filedialog.askopenfilenames(title="Select multiple files", filetypes=[("All Files", "*.*")], initialdir="/")
if not file_paths:
    sys.exit()
actual_paths = []
...
for i in extensions:
    for item in actual_paths: # iterdir iterates through files but not inside of sub directories and it returns a path instead of a string
        if item.is_file() and item.suffix.lower() == i: # is_file() is part of the pathlib module and it checks if something is a file, returning True or False
            ...
            print(item.name + " | " + i) # '.name' strips the rest of a path from an item. So if I have /Downloads/test.txt then '.name' would give me 'test.txt'
